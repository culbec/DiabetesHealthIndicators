...
class Scorer(object):
    ...
    def _regression_metrics(self, y_true: pd.Series, y_pred: pd.Series) -> dict[str, float]:
        ...
        if np.all(y_true >= 0) and np.all(y_pred >= 0):
            ...
            metrics["root_mean_squared_log_error"] = np.sqrt(skm.mean_squared_log_error(y_true, y_pred))
        ...
        metrics.update(
            {
                ...
                "root_mean_squared_error": np.sqrt(skm.mean_squared_error(y_true, y_pred)),
    ...
    def _classification_metrics(self, y_true: pd.Series, y_pred: pd.Series) -> dict[str, float]:
        ...
        return {
            ...
            "average_precision": skm.average_precision_score(y_true, y_pred) if not is_multi_class else None,
            ...
            "f1_samples": skm.f1_score(y_true, y_pred, average="samples") if is_multi_class else None,
            "neg_log_loss": skm.neg_log_loss(y_true, y_pred),
            ...
            "precision_samples": skm.precision_score(y_true, y_pred, average="samples") if is_multi_class else None,
            ...
            "recall_samples": skm.recall_score(y_true, y_pred, average="samples") if is_multi_class else None,
