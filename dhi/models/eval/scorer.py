...
class Scorer:
    ...
    def _compute_classification_metrics(
        ...
        metrics = {
            ...
            "precision_weighted": (
                em.precision_score(y_true, y_pred, average="weighted") if is_multi_class else None
            ),
        ...
        if beta != 1.0:
            if beta > 0.0:
                ...
                metrics["fbeta_micro"] = (
                    em.fbeta_score(y_true, y_pred, beta, average="micro") if is_multi_class else None
                )
                metrics["fbeta_macro"] = (
                    em.fbeta_score(y_true, y_pred, beta, average="macro") if is_multi_class else None
                )
